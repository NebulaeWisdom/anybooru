...
| [认证](authentication.md) | Danbooru 用 username + API key 走 HTTP Basic、Moebooru 用 password_hash、Serika 用 Bearer key、e621ng 同 Danbooru 但另一套引擎、Zerochan 没有认证只有 User-Agent 要求 |
| [分页](pagination.md) | 五个家族各自的页码参数与每页上限、游标形式、超过上限报什么错 |
...
2. **通用入口与原生方法**：五个客户端都有 `request()`，原生方法只是把参数拼好再调它。能传什么参数、
...
3. **返回什么就给你什么**：不自动翻页、不重试、不换别的接口重来；HTTP 非 2xx 时抛异常并保留状态码和正文。
   ...
   详情路径直接是条目对象。返回 JSON 的完整原貌、以及哪些方法不拆，见各家族方法参考。
...
- 依据分两条路：Danbooru / Moebooru / Serika / e621ng 对齐各自固定版本的上游源码（文件与行号见对应附注），
  Zerochan 没有源码可引用，只有官方 API 页面原文与真实响应。两条路都不是对每个下游站点的保证：
  站点可以自己关掉某个功能、按权限裁剪返回内容，也可能用反爬挡住你所在的网络。
- 已经真实执行过的匿名读取：Danbooru 的 12 次成功与 3 次预期错误、Moebooru 的指定匿名读取、Serika 的
  ...
  `entry_list` 与 `entry_show`。逐条命令、请求 URL 与结果见[验证记录](verification.md)。
- 只有源码依据、没有成功响应记录的部分：所有需要登录或 API key 的写路径、e621ng 需要成员权限的
  `related_tag` / `related_tag_bulk`、Serika 全部需 key 的 v1 方法。方法存在不等于测过。
- 两个“只读”家族要注意：e621ng 面没有原生写方法，写路由要用通用 `request()` 自己拼方法与路径；
  Zerochan 的 API 本身只读，且只提供 JSON（不实现 `xml`），文档要求的 User-Agent 里含项目名与
  Zerochan 用户名是站点约定，本库照配置原样发送、不校验、不代填。
