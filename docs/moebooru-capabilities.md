...
**不知道从哪个方法查起，先看这页；已经知道端点，再看 [Moebooru API 参数参考](moebooru-api.md)。**
...
- **有接口不等于本站启用，也不等于已经实测**。本文不新增线上探测；已实测范围单列在下方。

## 先按目的找入口

下表的 `c` 是已按根配置创建的 `Moebooru` 客户端；`tags`、各类 ID 等代表你从配置或前一次响应取得的值。
...
| 下载原图或预览图 | `post_list` / `pool_show` 返回的 `file_url` / `sample_url` / `preview_url`；保存文件需另用 HTTP 客户端，本库没有原生下载方法 | `post = c.post_list(tags='id:{0}'.format(post_id))[0]` | 地址取决于帖子对当前用户是否可见 |
| 按作者找作品 | `post_list`：已知画师标签时直接当标签搜索 | `c.post_list(tags=artist_name)` | 匿名可查 |
| 查画师记录 | `artist_list`：按名字前缀或主页 URL 过滤 | `c.artist_list(name=name)`；`c.artist_list(url=url)` | 匿名可查 |
...
| 看评论、笔记与它们的版本 | `comment_list` / `comment_search` / `note_list` / `note_search` / `note_history` | `c.comment_list(post_id=post_id)`；`c.comment_search(query='')`（不带 `post_id` 的评论流，可能为空）；`c.note_list(post_id=post_id)` | 匿名可读；发表评论、改笔记需登录 |
...
| 审核待删帖、释出被 hold 的帖子 | `post_moderate` / `post_activate` / `post_acknowledge_new_deleted_posts` | `c.post_moderate(ids={post_id: 1}, commit='Approve')`；`c.post_activate([post_id])` | 审核需 janitor；释出 hold 需成员，非版主仅限自己的帖子；已读提示动作对匿名只返回成功、不更新用户状态 |
...
**四个容易选错的地方**：Moebooru **没有** `search` 字典，过滤条件放在顶层；**没有 `post_show` / `wiki_show`**
这两个 JSON 方法，单帖用 `tags='id:<id>'`、wiki 用 `wiki_list` 或 `wiki_history`；`comment_list` **必须带
`post_id`**（否则返回空数组），要按分页读评论流用 `comment_search(query='')`（空列表是正常结果）；各端点的
每页数量由服务端写死，`comment_list`、`note_history`、`artist_list` **会忽略 `limit`**
（见 [API 参考的上限表](moebooru-api.md#分页与实际上限)）。
...
| 身份 / 条件 | 可以期待的能力 | 主要边界 |
| :--- | :--- | :--- |
| 匿名 | 帖子、标签、相关标签、别名与蕴含列表、画师列表、wiki 列表/版本/最近改动、评论、笔记、合集、论坛、用户列表、收藏者名单、热门帖、相似图 | 内容可见性、私密设置、反垃圾过滤器（`url1`）仍生效 |
| 已登录普通账号 | 上传、编辑帖子、投票、标记、评论、笔记、合集维护、wiki 编辑、论坛发帖、收藏相关写操作 | 登录不是全站写权限；还要看对象归属、帖子锁定状态与账号等级 |
| contributor / privileged / janitor / mod / admin | 合集复制、画师删除、恢复与审核、wiki 锁定与删除、论坛主题管理、标签批量改名、用户记录删除 | 各动作要求的等级不同，逐条见 [API 参考](moebooru-api.md) |
...
匿名只读端点在 `yande.re` 上的执行记录写在 [verification.md](verification.md)（由维护者追加逐条结果），
本文不重复、也不虚构任何计数。

其他能力列在这里，是因为**原生方法与对应上游契约存在**（本地 `moebooru/` HEAD `206455e1` 的路由与控制器），
不是宣称全部实测通过。**全部写接口都只做源码对齐，未做线上实测**；账号类动作（`user_check`、
`user_create`、`user_reset_password`）没有可用凭据，同样只保证请求形态与源码一致。
本次也没有为了编写总览增加线上请求。

## 完整原生方法索引

以下按资源列出 `pybooru/api_moebooru.py` 的 **90 个原生 API 方法**；每项只解释“干什么”，参数与路由细节见
[Moebooru API 参数参考](moebooru-api.md)。多数写动作需要登录或更高等级；账号等例外须看各自权限。
...
**90 个方法只覆盖有 JSON 契约的端点。** 浏览型页面、JS 交互、Atom/RSS 订阅源与合集 ZIP
都被有意排除（都有真实路由，只是没有 JSON）：完整清单与原因见
[API 参考的排除项](moebooru-api.md#排除项不是缺失路由)。需要时可以直接用通用入口：

```python
client.request('GET', 'post/popular_recent', params={'period': '1w'})
```

`request()` 不会把 HTML 变成 JSON，也不会替你跟随“写操作”的重定向做成功判定。
客户端还提供 `close()` 与 `with Moebooru(...) as c` 管理连接生命周期，这两个不算 API 方法。

继续阅读：[客户端用法](moebooru.md) · [完整端点与参数](moebooru-api.md) ·
[翻页方式](pagination.md) · [错误与权限失败](errors.md)。
