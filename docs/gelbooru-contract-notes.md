...
| L：本轮真实响应 | [验证记录](verification.md#gelbooru匿名只读实测2026-09-18)，包含命令、URL、状态和返回关键内容 | 1 次客户端补全，4 个 HTML 页面与 1 个 JavaScript 文件；dapi 为零次 |
...
| :--- | :--- | :--- |
| `post_list` | W 确认可按帖子 id 查询；T 转述帖子网页含 ID、Posted、Uploader、宽高、评级、分数、来源、标签、三类图片地址。本轮没有重读该图片网页 | `id/created_at/owner/width/height/rating/score/source/tags/file_url/sample_url/preview_url` 是候选命名，**未实测（需账号）** |
...
| 项目 | 原文 / 既有说法 | 本轮处理 |
| :--- | :--- | :--- |
| dapi 匿名权限 | W：“We will occasionally require authentication”；T：匿名一律 401、空正文 | 本轮以账号所需路径实现，五个方法不联网；401 只标为 T 的既有记录，不计本轮成功或错误次数 |
...
| 未知 autocomplete type | T 参数说明说未知值返回 `[]`，其记录却说 `type=wiki` 回退标签 | 两种说法并存，不选一条做保证；本轮只跑 type=tag |
...
* 真实请求只含补全、标签列表、标签蕴含、官方 wiki、旧 help 和站点 JS。完整命令、次数和 URL 在[验证记录](verification.md#gelbooru匿名只读实测2026-09-18)，不把资料里的旧记录改成本轮实测。
* `post_list` / `post_deleted` / `tag_list` / `user_list` / `comment_list` 全为未执行 / 未实测（需账号）；认证成功、JSON 字段与包装、排序分页、错误体、配额、限流都没有本站证据。
* 其它补全 type、别名方向、空值、未知值、空格输入与 limit 边界没有本轮证据。站点 JavaScript 是客户端消费者，不是服务端存在性证明。
* 不实现 XML、HTML 解析、账号登录、写请求；也不封装官方未列出的 `s=artist/pool/wiki`。其它部署、CDN 下载或地址规律未验证。
