...
`Moebooru` 客户端有 **90 个原生方法**。每个方法只做一件事：按上游 Moebooru 引擎（只读参考
`moebooru/` HEAD `206455e1`）确实提供 JSON 的路由把参数发出去，并把返回的 JSON 交给你。
本页给每个方法的请求路径、参数（含默认值与不传时的行为）、返回字段、需要的身份，常用方法配可复制的字面调用。
逐条权限过滤器、上游文件行号与被排除的浏览器页面在[契约审计附注](moebooru-contract-notes.md)；“我要做什么 → 用哪个方法”在[能力总览](moebooru-capabilities.md)。

## 本页怎么用

**每个代码块都能单独跑**：自己 import、自己 `with Moebooru('yandere') as client:`，参数写成字面值，不需要额外配置键。
块内变量名就是它装的内容（`posts` 装帖子数组、`post_id` 装一个帖子编号）。
标“需要登录”的块照原样发出去会被服务端拒绝或改变线上数据，**本轮一次都没有执行**，只用来表明参数与请求体形状。
...
默认匿名：构造时 `username` 与 `password` 都空，请求里没有 `login` 与 `password_hash`。
...
页码从 1 开始，服务端把 `page` 夹在 `1..1000000`。每页条数由服务端写死，下表是引擎的真实行为，不是客户端的限制。
标“`limit` 不读”的端点传了也没用。
...
with open('image.jpg', 'rb') as upload_file:
    with Moebooru('yandere') as client:
        ...
        # -> {'success': True, 'post_id': <新帖编号>, 'location': 'post/show/<新帖编号>'}
```
...
| :--- | :--- | :--- | :--- | :--- |
| `post_list(**params)` | `GET post.json`（旧版本补成 `post/index.json`） | `tags` 站点标签语言，元标签全可用；`limit` 默认 40、上限 1000；`page` 默认 1；`filter='1'` 打开可见性过滤（`.json` 调用默认跳过 `can_be_seen_by?`，带了它才按当前身份的可见性设置过滤）；`api_version='2'` 换结构；`include_tags` / `include_votes` / `include_pools='1'` 在 v2 结构里多带对应字典 | 数组，字段见上面的块；已删除帖没有 `file_url` / `sample_url` / `jpeg_url` | 匿名 |
| `post_create(tags, *, file=None, source=None, md5=None, anonymous=None, **attributes)` | `POST post/create.json`（multipart） | `tags` 空格分隔标签串；`file` 打开的文件对象；`source` 来源地址，只有它也能上传（服务端自己下载）；`md5` 上传后校验，不匹配会销毁帖子并回 420；`anonymous='1'` 隐藏上传者（contributor）；`post[rating]` / `post[parent_id]` / `post[is_held]` 等属性 | `{'success': True, 'post_id': <新帖编号>, 'location': 'post/show/<新帖编号>'}`（开启上传查重、命中相似图时另带 `has_similar_hits` / `similar_location`）；重复 md5 → 423，超出每日上传限额 → 421 | 需登录（成员） |
...
| `post_vote(post_id, score=None)` | `POST post/vote.json` | `score`：`1` 好 / `2` 很好 / `3` 收藏 / `0` 清除；不传就是只读当前投票；`score > 3` 对任何等级都 → 424，负分只对非版主 → 424 | 只读 `{'success': True, 'vote': 1}`；写入回被触及帖子的批量数据 | 需登录 |
...
| `pool_order(pool_id, sequences)` | `POST pool/order.json` | `sequences` 是 `{合集内帖子记录编号: 顺序}` | 同样恒重定向，读到合集 JSON；**不是写入确认** | 路由未设该登录过滤（不在 `member_only` 列表里）；动作内按 `can_be_updated_by?` 判断：公开合集或当前用户对该合集有权限时继续，否则拒绝；匿名能否成功**未实测** |
...
| `note_update(note_id=None, **attributes)` | `POST note/update.json` | 判别看 `note[post_id]`：带了它就新建（新建时必给 `note[post_id]`），没带才按 `id` 更新已有笔记；**同时给 `note_id` 与 `note[post_id]` 会新建一条，`note_id` 被忽略**；属性 `note[x]`、`note[y]`、`note[width]`、`note[height]`、`note[body]`、`note[is_active]='1'/'0'` | `{'success': True, 'new_id': …, 'old_id': …, 'formatted_body': …}`；锁定帖 → 422 | 需登录 |
...
| 方法 | 请求 | 参数（默认值 / 不传时） | 返回 | 身份 |
| :--- | :--- | :--- | :--- | :--- |
| `artist_list(**params)` | `GET artist.json` | `name`（前缀匹配：服务端发 `name LIKE '<前缀>%'`，不是精确相等）、`url`、`order`（`name` 默认 / `date`）、`page`；每页 25，带 `name`/`url` 时 50；`limit` 不读 | 数组：`id`、`name`、`alias_id`、`group_id`、`urls`（数组） | 匿名 |
...
| `tag_related(**params)` | `GET tag/related.json` | `tags` 空白分隔（服务端剥掉 `%` 与 `,` 并转小写，`*`、`/` 原样保留）、`type` 必须是站点 tag_types 的键；没有 `limit` | 对象 `{'查询标签': [['名称', 共现次数], …]}`，每组最多 25 项 | 匿名 |
...
| `wiki_create(title, body)` | `POST wiki/create.json` | `wiki_page[title]`、`wiki_page[body]` | `{'success': True}`（**没有 `location`**：控制器把它当 `respond_to_success` 的位置参数传入，JSON 分支只读 `api` 选项）；校验失败 → 420 | 需登录（成员） |
...
* 本页每个方法的参数、返回字段与身份都按上游源码对齐。**写动作与账号动作没有线上实测**；匿名只读的实际执行范围与逐条命令见 [verification.md](verification.md)，逐条状态与上游行号见[契约审计附注](moebooru-contract-notes.md)。
* 只包装有 JSON 的端点：HTML / JS 页面、Atom/RSS 订阅源与合集 ZIP 都没有方法，它们仍是有真实路由的。其中一个特殊情况是 `help/api`——本库固定发 `Accept: application/json`，而该页只有 HTML 模板，所以 `request('GET', 'help/api')` 会拿到 404（yande.re 是它的 HTML 404 页，konachan.com 与 sakugabooru.com 是空正文），把 Accept 换成 `text/html` 就是 200。完整排除清单见附注。
* 方法存在不等于目标站点启用了该能力：相似图服务、异步任务后端、站点自己关掉的功能都可能缺失，结果由服务端决定，本库不做本地补丁或替代实现。
