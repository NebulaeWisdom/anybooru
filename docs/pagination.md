...
* 分页参数（多数家族为 `page` / `limit`，Zerochan 为 `p` / `l`）原样传给服务端，不设本地上限、不裁剪、不补默认值；
...
`entry_list` 用 `p`（页码）与 `l`（每页条数），不是 `page` / `limit`。API 页面给出 `l` 范围
`1–250`，没有公布缺省值；客户端不替服务端设置或钳位这些参数。
列表 JSON 是 `{"items": [...]}`，原生方法返回其中的数组；已观察响应没有 `total` / `page` 等分页元数据，
库不构造总数或下一页游标。实际默认条数与分页请求记录见
[Zerochan 验证](verification.md#zerochan匿名只读实测2026-09-18)，参数契约见
[Zerochan 方法参考](zerochan-api.md)。未实测末页、越界页码或非法 `l` 的错误行为。
