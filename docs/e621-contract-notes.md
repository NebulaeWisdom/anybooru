...
[能力入口](e621-capabilities.md)）只保留可操作的事实；本文记上游文件与行号、权限过滤器、
源码与部署的差异、排除项与逐条状态。
...
→ `app/blueprints/*.rb` 与 `app/models/*`（返回结构、搜索、校验）→ `config/danbooru_default_config.rb`（默认值）。
客户端侧依据是 `anybooru/api_e621.py`（`E621Api_Mixin`，18 个原生方法）与 `anybooru/e621.py`（客户端与
`request()`）。

**目录形态与 Danbooru 不同**：e621ng 没有 `app/policies/` 与 `app/serializers/`。权限过滤器由
`app/controllers/application_controller.rb` 用 `define_method` 生成，帖子返回结构在 `app/blueprints/*.rb`。
`/help/api` 属于 `help_pages` 路由而不是 wiki（下文单列）。

## 状态口径与总量

* 客户端原生方法 **18 个**，全部 `GET` 只读，每个方法体内调用一次 `E621.request(...)`。
* **已有匿名只读记录（2026-09-15 与 2026-09-18，`e621.net` 与 `e926.net`）**：
  ...
  * 返回结构复核 **7 次调用**：两个站点的 `post_count`（`200`，`{"count": 240001, "capped": true}`）、
    原始正文 `{"posts": [...]}`（`200`）、`md5` 单帖（`200`）、`only` 分支（`200`）、`v2=true` 分支（`200`）、
    `related_tag` 匿名（`403`）；
  * 命令、URL、状态码与响应摘要在[验证记录](verification.md#anybooru-改名后的复跑2026-09-18)。
* **只有源码依据、未实测**：`related_tag` 与 `related_tag_bulk` 的成员成功路径（未配置认证凭据）；
  `v2=true` 的 `mode=extended` / `mode=thumbnail` 两个视图；全部写路径、staff 面与只读扩展路由。
  `related_tag_bulk` 没有任何线上调用（连失败响应也没有）。
* “已实测”只覆盖当时那次调用用到的参数组合；同一方法换参数、换身份、换站点都不算已验证。
  匿名 `403` 是权限边界的证据，不是成功路径。
* 本轮字面代码复核另有 31 次调用：30×200、1×404（不存在的帖子0）。新增页2、按名称/分类筛标签、
  按主页筛画师、按帖子分组查评论、按类别筛合集的响应；按帖子分组实际返回5帖，调用里的limit=2没有改变这点。
  逐条执行见[文档代码复核](verification.md#文档字面代码复核2026-09-18)。
...
| 方法 | 路由（`GET`） | routes.rb | 控制器 | 匿名 | 返回给 Python 的东西 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| `post_list(**params)` | `posts.json` | 339（集合 344-347） | `posts#index` 11-62 | 是（过滤器例外清单在 6、8） | 默认从 `{"posts": [...]}` 取数组；`md5` 命中时从 `{"post": {...}}` 取对象；`only` / `v2=true` 时不取，直接给列表或对象 |
| `post_show(post_id, **params)` | `posts/<id>.json` | 339 | `posts#show` 77-104 | 是 | 默认从 `{"post": {...}}` 取对象；`only` / `v2=true` 时不取 |
| `post_random(**params)` | `posts/random.json` | 344-347 | `posts#random` 171-181 | 是 | 同 `post_show`；无匹配 `404` |
| `post_count(**params)` | `posts/count.json` | 344-347 | `posts#count` 64-75 | 是 | `{"count": N, "capped": bool}` 原样（上游就是内联对象） |
| `tag_list(search=None, **params)` | `tags.json` | 392 | `tags#index` 8-12 | 是 | 标签对象数组 |
| `tag_show(tag_id, **params)` | `tags/<id>.json` | 392 | `tags#show` 14-21 | 是 | 单个标签对象；数字按 id、否则按名称精确查，未命中 `404` |
| `artist_list(search=None, **params)` | `artists.json` | 209 | `artists#index` 11-30（JSON 分支 23-29） | 是（7） | 画师对象数组，每项带 `urls` |
| `artist_show(artist_id, **params)` | `artists/<id>.json` | 209 | `artists#show` 32-56 | 是（7） | 画师对象，带 `urls` 与 `domains` |
| `comment_list(search=None, **params)` | `comments.json` | 229 | `comments#index` 13-19 → 105-113 / 115-141 | 是（7，`skip_before_action :api_check` 在 11） | 评论对象数组；`group_by=post` 时是帖子对象数组 |
| `comment_show(comment_id, **params)` | `comments/<id>.json` | 229 | `comments#show` 21-26 | 是（7） | 单个评论对象；不可见 `403` |
| `pool_list(search=None, **params)` | `pools.json` | 311 | `pools#index` 9-17 | 是（5） | 合集对象数组（含 `creator_name`、`post_count`） |
| `pool_show(pool_id, **params)` | `pools/<id>.json` | 311 | `pools#show` 19-26 | 是（5） | 单个合集对象 |
| `note_list(search=None, **params)` | `notes.json` | 302 | `notes#index` 10-15 | 是（5） | 笔记对象数组（含 `creator_name`） |
| `note_show(note_id, **params)` | `notes/<id>.json` | 302 | `notes#show` 17-22 | 是（5） | 单个笔记对象 |
| `wiki_page_list(search=None, **params)` | `wiki_pages.json` | 451 | `wiki_pages#index` 9-26 | 是（5） | 页面对象数组（含 `creator_name`、`category_id`） |
| `wiki_page_show(title_or_id, **params)` | `wiki_pages/<id>.json` | 451 | `wiki_pages#show` 28-45 | 是（5） | 单个页面对象；数字按 id、否则 `WikiPage.titled`（56-58），未命中 `404` |
...
`Tag.find_by!(name: params[:id])`，不归一化。列表搜索不同：`app/models/tag.rb:353-357` 的 `name` 与
`app/models/tag.rb:341-343` 的 `name_matches` 会先调用 `normalize_name`（`app/models/tag.rb:190-192`，
小写化、去空白、空格转下划线），`fuzzy_name_matches` 则原样传值（`app/models/tag.rb:337-339`）。
画师名称查找走 `Artist.named`（`app/models/artist.rb:421-423`），由 `app/models/artist.rb:265-267`
小写化、去首尾空白并转下划线。这些名称分支只有源码依据；`tag_show` 未命中由
`app/controllers/application_controller.rb:145-146` 转入 `render_404`（`app/controllers/application_controller.rb:178-190`）。

两条相关标签路径的 `member_only` 是控制器级全局过滤器（`related_tags_controller.rb:6`），所以权限失败先于
参数解析。`routes.rb:382` 的 `resource :related_tag, only: %i[show update]` 另外登记了 `PUT /related_tag`，
但控制器没有 `update` 动作，该动词只会落进错误页，属应排除的动词。`related_tag` 的动作直接读
`params[:search][:query]` 与 `params[:search][:category_id]`（`related_tags_controller.rb:9`），
缺 `search` 不会抛异常：全局 `before_action :normalize_search`（`application_controller.rb:35`）
在 346-377 行先执行 `params[:search] ||= ActionController::Parameters.new`（360 行），把缺失的 `search`
补成空对象，动作读到 `nil`（等于空查询），这次补空也不会触发它自己的 `302` 清理；匿名请求更早就在
`member_only` 处得到 `403`。`bulk` 读顶层 `query` / `category_id`（18-19），与 `search` 无关。

<a id="sec-post-shapes"></a>

## 帖子返回结构：由请求参数决定

不是调用者随便挑的开关，而是上游按参数分支选择：

| 输入 | 生效处 | 结果 |
| :--- | :--- | :--- |
| 无 `md5`，且 `only` 为空、未给 `v2=true` | `posts_controller.rb:56-58` → `json_response_helper.rb:39-42` | 旧结构数组，外层是 `{"posts": [...]}` |
| 同上但带 `only=`（任意非空值） | `json_response_helper.rb:13-15` | 同一份旧结构**直接输出**，没有外层字典，字段**不裁剪** |
| `md5=<非空字符串>` | `posts_controller.rb:12-21`（`pick_json_format(..., collection: false)` 在 18） | `Post.find_by!(md5:)`；未命中 `404`；默认外层是 `{"post": {...}}` |
| `v2=true` | `json_response_helper.rb:44-56` | 新结构，**永远没有外层字典**（集合就是数组） |
...
配套事实：`v2` 只有精确字符串 `"true"` 才走新结构；`mode` 在旧结构分支被完全忽略；`only` 在新结构分支
被忽略；带 `only` 时只去掉外层，不做字段筛选（`json_response_helper.rb:13-20`）。
...
* 其它资源走模型默认 `as_json`：`ApplicationRecord::ApiMethods`（`app/models/application_record.rb:188-212`）
  ...
  画师还声明了非表列属性 `notes`（`app/models/artist.rb:37` 的 `attribute :notes, :string`），由上述默认
  属性序列化一同返回；未填写时为 `null`。

帖子字段与可见性强相关，要区分两条序列化路径：
...
* **`/posts.json` 的旧结构蓝图**：不复用上面那层，`file.md5` / `file.ext` 仍取自模型；只有 `file.url`、
...
所以“不可见”在帖子返回里的表现是**键还在、值为空**，而不是键被删掉；新结构同理，只在
`post.visible?` 为真时填 `files.original.url` 等地址（`app/blueprints/post_blueprint.rb:16-67`）。

<a id="sec-client"></a>

## 客户端侧实现契约

`anybooru/e621.py` 与 `anybooru/api_e621.py` 只做三件与上游一一对应的事，其余原样交回：
...
* **取键**：`envelope` 只接受一个键名，方法内按上面的判定写死 `posts` / `post` / 不传；键不存在直接
  `KeyError`，不会去找别的键——上游分支与返回值一一对应，不引入第二套猜测规则。
* **参数组装**：`_search(search, params)` 把搜索字典整包放进 `search`（对应 `search_params` 的
  ...
  `/`）；`request()` 负责补 `.json`、去掉开头 `/`、把布尔编码成小写字符串、省略 `None`，并在 `username`
  或 `api_key` 非空时才加 Basic 头。
...
数值与时间收 `5`、`>5`、`5..10`、`5,6,7`，布尔按 truthy/falsy 字符串，文本含 `*` 走 `LIKE`、否则走
...
**空的 `search` 值会被重定向**：`normalize_search`（346-377）对所有 `GET`/`HEAD` 请求剔除空白 `search` 项
（含嵌套），只要有东西被剔掉就 `302` 到清理后的地址（`wiki_pages#index` 另有 `normalize_search_params`
把顶层 `title` 搬进 `search[title]`，`wiki_pages_controller.rb:121-126`）。
...
* **过滤器**：`member_only`、`staff_only`、`admin_only` 等由 `UserLevel::ROLES` 循环 `define_method`
  生成（`application_controller.rb:289-305`），实际含义是“该等级及以上”；匿名不满足 `is_member?`，
...
权限与错误正文由 `ApplicationController` 决定（`app/controllers/application_controller.rb`）：
...
2xx 但不是 JSON 抛 `AnybooruAPIError`；`403` 的正文实测为
...
* 计数：`posts#count`（`posts_controller.rb:64-75`）用
  ...
  `count_capped?`（`document_store_extensions.rb:61-74`）。所以 `capped` 表示服务端返回的是下限；
...
* 词表只有 `s` / `q` / `e`：元标签解析处 `app/logical/tag_query.rb:1454`
  （`if %w[s q e].include?(g2 = g2[0]&.downcase)`）——首字母不在表内的 `rating:` 元标签被整条丢弃，
...
* 安全模式：`SessionLoader#set_safe_mode`（`session_loader.rb:250-253`）取
  ...
  （`danbooru_default_config.rb:27-30`），可用部署自己的 `config/danbooru_local_config.rb`
  （`config/application.rb:24` 引入，上游 `.gitignore:5` 忽略该文件）或 `DANBOORU_SAFE_MODE` 环境变量覆盖。
...
* 实测区分：`/help/api` 返回 HelpPage（`name` 为 `api`，`wiki_page` 指向 `e621:api`）；
  ...
  先前的匿名探测给 `/help/api` 发送 `Accept: application/json` 时取得了 HelpPage JSON。
...
标签关联与集合类 `/tag_aliases.json`、`/tag_implications.json`、`/artist_urls.json`、`/post_sets.json`、
...
* **可以匿名读、但没有原生方法的 JSON 路由**：`artists/show_or_new` 走
  ...
  18 个原生方法里；需要时用 `request()`，本路线只有源码依据、未实测。
* **不存在的路由**：`/counts/*`（Danbooru 的形状）在上游没有对应路由；`/post` 系列单数路径在源码里是 `302`
  重定向（`routes.rb:569`、`578-583`），目标多数不带 `.json`，不可当作入口。
...
以下都是**部署侧观察**（匿名只读），不是上游源码的结论：

* `/post.json` 返回 `403` HTML「Access Denied - Oh no!」反脚本页：该页面上游仓库里不存在，源码侧这条路径
  是 `302` 重定向。因此任何“复数/单数”判别式在 e621 都不成立——须以站点自述与一次真实 JSON 响应判断引擎。
* `/counts/posts.json` 返回站点自己的 HTML `404`：说明的只是“没有 Danbooru 的计数路由”，**不代表**没有计数
  能力（真实计数是 `/posts/count.json`）。
* `rating:g` 与不传 `tags` 返回同一帖，与源码的静默丢弃一致。
* `/help/api` 的实体是 HelpPage，`/wiki_pages/help%3Aapi.json` 是 WikiPage（见上一节）。
* e926 与 e621 的 JSON 面都可匿名使用；e926 的 HTML 路由被 Cloudflare 挑战页挡住，
  这是网络侧现象，不能泛化成“e926 不可用”。
...
* 客户端沿用配置中的描述性 `User-Agent`（包内默认 `Anybooru/0.1.0.dev1`）。此快照未检出入站 User-Agent 的
  强制校验；`config/danbooru_default_config.rb:735-741` 定义的是站点**出站** HTTP 头，
...
* `related_tag` / `related_tag_bulk` 的成员成功路径（含 `category_id` 分支与 `bulk` 的 `POST` 变体）——
  成员身份需要凭据；两者成功路径只有源码依据，未实测。
* `v2=true` 的 `mode=extended` 与 `mode=thumbnail` 两个视图、不可见身份下帖子字段的置空与隐藏分支、
  按名称查标签/画师、`search` 的成员/staff/admin 专用条件；按帖子分组的评论已在本轮匿名执行。
* 全部写路径、staff 面、OAuth 面、上面列出的只读扩展路由、以及自托管实例的差异。
* 本轮没有运行 formatter / lint / 测试套件；上游自带的 specs 只作为源码依据的第二份证据被引用。
