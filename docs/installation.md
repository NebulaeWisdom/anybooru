...
默认读随包安装的 `anybooru/anybooru.json`（里面有 15 个站点条目 `serika`、`danbooru`、`safebooru`、
...
| 路径 | 说明 |
| :--- | :--- |
| `anybooru/` | 包源码：`danbooru` / `moebooru` / `serika` / `e621` / `zerochan` / `gelbooru` / `gelbooru02` / `shuushuu` / `sakuria` / `anime_pictures` / `cosine` 各有客户端模块与 `api_<family>.py` 方法模块，`anybooru.py` 为共享核心 |
...
| `anybooru/anybooru.json` | 随包默认配置：`request`（超时、代理、User-Agent）、`sites`（15 个站点条目）、`examples`、`smoke`、`verification` |
...
Zerochan、Gelbooru、Gelbooru02（TBIB）、Shuushuu、Sakuria、Anime-Pictures 与 Cosine 都没有可供核对的
...
[Anime-Pictures 契约审计附注](anime-pictures-contract-notes.md) 与
