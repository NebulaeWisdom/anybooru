...
默认读随包安装的 `anybooru/anybooru.json`（里面有 14 个站点条目 `serika`、`danbooru`、`safebooru`、
...
| 路径 | 说明 |
| :--- | :--- |
| `anybooru/` | 包源码：`danbooru` / `moebooru` / `serika` / `e621` / `zerochan` / `gelbooru` / `gelbooru02` / `shuushuu` / `sakuria` / `anime_pictures` 各有客户端模块与 `api_<family>.py` 方法模块，`anybooru.py` 为共享核心 |
...
| `anybooru/anybooru.json` | 随包默认配置：`request`（超时、代理、User-Agent）、`sites`（14 个站点条目）、`examples`、`smoke`、`verification` |
...
Zerochan、Gelbooru、Gelbooru02（TBIB）、Shuushuu、Sakuria 与 Anime-Pictures 都没有可供核对的本地上游服务端源码：Zerochan 的依据是
...
（输入引用的外部客户端源码链接本轮没有独立读过）。六者都不从其他引擎推断。
...
[Sakuria 契约审计附注](sakuria-contract-notes.md) 与
