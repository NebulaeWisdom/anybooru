# -*- coding: utf-8 -*-
"""Anonymous smoke checks for yande.re (Moebooru engine).
...
downloads, no extra requests:
  ...
  4. comment_show(0) -> 404; the file requires the AnybooruHTTPError and that no
     JSON body was parsed (data is None). This site answers the unknown comment
     id with an HTML page, so the body text is left unchecked.
...
sites.yandere, and every query value comes from the root smoke section.
"""
...
def exercise(client, settings):
    ...
    def inspect_missing(error):
        require(error.data is None, f'expected no JSON error body, got {type(error.data).__name__}')
        return f'AnybooruHTTPError; data=None; body_chars={len(error.body)}'
    ...
    check(f'comment_show {settings["missing_id"]}',
          lambda: client.comment_show(settings['missing_id']), inspect_missing, expected=404)
...
def main():
    ...
    try:
        ...
        client = Moebooru('yandere', username='', password='',
                          config_file=args.config)
        settings = client.config['smoke']
    except Exception as error:
        print(f'FAIL setup | - | no HTTP | {type(error).__name__}: {error}')
        print('SUMMARY yandere | requests=0 | passed=0 failed=1')
        return 1
    with client:
