# -*- coding: utf-8 -*-
"""Anonymous smoke checks for e926.net (e621ng engine).
...
Same e621-family parameters as the smoke section's e621 entry are used.
...
with success=false plus a reason string. Post payloads are the nested legacy
shape: file dimensions/format/size and tag categories, with file.url either a
string or null for a legitimately unavailable file.
"""
...
def exercise(client, settings):
    ...
    safe_tags = settings['e621']['tags']
    limit = settings['limit']
    missing_id = settings['missing_id']

    def inspect_file(post):
        file = post['file']
        require(type(file) is dict, f'file: expected object, got {type(file).__name__}')
        require('url' in file, 'file: missing field url')
        require(file['url'] is None or type(file['url']) is str,
                f'file.url: expected str or None, got {type(file["url"]).__name__}')
        fields(file, {'width': int, 'height': int, 'ext': str, 'size': int})
        return (f'{file["ext"]} {file["width"]}x{file["height"]} size={file["size"]}'
                f' url={"set" if file["url"] else "none"}')

    def inspect_tags(post):
        tags = post['tags']
        require(type(tags) is dict, f'tags: expected object, got {type(tags).__name__}')
        require('general' in tags, 'tags: missing category general')
        for category, names in tags.items():
            require(type(category) is str,
                    f'tags key: expected str, got {type(category).__name__}')
            require(type(names) is list,
                    f'tags[{category}]: expected list, got {type(names).__name__}')
            for name in names:
                require(type(name) is str, f'tags[{category}]: expected string entries')
        return f'{len(tags)} categories, general={len(tags["general"])}'

    def inspect_post(post):
        require(type(post) is dict, f'expected post object, got {type(post).__name__}')
        detail = fields(post, {'id': int, 'rating': str})
        return f'{detail} file={inspect_file(post)} tags={inspect_tags(post)}'
    ...
    def inspect_missing(error):
        ...
        require(type(error.data.get('reason')) is str, 'body reason is not a string')
        return f'success=false reason={error.data["reason"]}'
...
def main():
    ...
    try:
        ...
        client = E621('e926', username='', api_key='', config_file=args.config)
        settings = client.config['smoke']
    except Exception as error:
        print(f'FAIL setup | - | no HTTP | {type(error).__name__}: {error}')
        print('SUMMARY e926 | requests=0 | passed=0 failed=1')
        return 1
    with client:
