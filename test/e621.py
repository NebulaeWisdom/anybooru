# -*- coding: utf-8 -*-
"""Anonymous smoke checks for e621.net (e621ng engine).
...
def main():
    ...
    try:
        ...
        client = E621('e621', username='', api_key='', config_file=args.config)
        settings = client.config['smoke']
    except Exception as error:
        print(f'FAIL setup | - | no HTTP | {type(error).__name__}: {error}')
        print('SUMMARY e621 | requests=0 | passed=0 failed=1')
        return 1
    with client:
