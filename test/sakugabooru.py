# -*- coding: utf-8 -*-
"""Anonymous smoke checks for sakugabooru.com (Moebooru engine).
...
downloads, no extra requests:
  ...
  4. comment_show(0) -> 404; the file requires the AnybooruHTTPError and that no
     JSON body was parsed (data is None). The body text itself is not asserted.
...
sites.sakugabooru, and every query value comes from the root smoke section.
"""
...
def main():
    ...
    try:
        ...
        client = Moebooru('sakugabooru', username='', password='',
                          config_file=args.config)
        settings = client.config['smoke']
    except Exception as error:
        print(f'FAIL setup | - | no HTTP | {type(error).__name__}: {error}')
        print('SUMMARY sakugabooru | requests=0 | passed=0 failed=1')
        return 1
    with client:
